    ...
    >
      <div className="grid gap-6 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] lg:gap-8">
        <div className="landing-accent-panel rounded-[2.5rem] p-6 sm:p-8">
          ...
          <p className="mt-6 max-w-xl text-4xl font-heading leading-tight tracking-tight text-primary-foreground">
          ...
          <div className="mt-8 grid gap-3">
            {resultProof.map((item) => (
              <div key={item.label} className="rounded-[1.6rem] border border-white/16 bg-white/10 p-4">
                <p className="text-sm font-medium text-primary-foreground/70">{item.label}</p>
                <p className="mt-2 text-base leading-7 text-primary-foreground">{item.value}</p>
              </div>
            ))}
          </div>
        </div>
